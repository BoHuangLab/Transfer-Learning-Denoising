...
def load_denoising(
    ...
    if transform is None:
        ...
        transform = transforms.Compose(
            [transforms.CenterCrop(patch_size), fluore_to_tensor]
        )
    target_transform = transform

    dataset = DenoisingFolder(
        root,
        train,
        noise_levels,
        types=types,
...
def load_denoising_test_mix(
    ...
    # the same
    target_transform = transform
    dataset = DenoisingTestMixFolder(
        root, loader, noise_levels, transform, target_transform
    )
    kwargs = {"num_workers": 4, "pin_memory": True} if torch.cuda.is_available() else {}
    data_loader = DataLoader(
        dataset, batch_size=batch_size, shuffle=True, drop_last=False, **kwargs
...
if __name__ == "__main__":
    ...
    transform = transforms.Compose(
        [
            ...
            transforms.Lambda(
                lambda crops: torch.stack([fluore_to_tensor(crop) for crop in crops])
            ),
            transforms.Lambda(lambda x: x.float().div(255).sub(0.5)),
        ]
    )
