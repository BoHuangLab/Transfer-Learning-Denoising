...
def build_3d_conv_unet(inputs, use_bias=True):
    def flatten_time(layer, padding_mode="same", use_bias=True):
        ...
        layer = ks.layers.Conv3D(
            filters=num_channels,
            padding=padding_mode,
            kernel_size=[n_time, 1, 1],
            activation="relu",
            use_bias=use_bias,
        )(layer)
    ...
    def u_net_down_block(
        layer, index, max_pool=True, padding_mode="same", use_bias=True
    ):
    ...
    def u_net_up_block(layer, concat_tensor, padding_mode="same", use_bias=True):
        ...
        layer = ks.layers.Conv2DTranspose(
            filters=num_channels,
            kernel_size=2,
            strides=(2, 2),
            padding=padding_mode,
    ...
    padding_mode = "same"
    # downsampling path
    conv0_1 = u_net_down_block(
        inputs, index=0, max_pool=False, padding_mode=padding_mode, use_bias=use_bias
    )
    conv1_1 = u_net_down_block(
        conv0_1, index=1, padding_mode=padding_mode, use_bias=use_bias
    )
    conv2_1 = u_net_down_block(
        conv1_1, index=2, padding_mode=padding_mode, use_bias=use_bias
    )
    conv3_1 = u_net_down_block(
        conv2_1, index=3, padding_mode=padding_mode, use_bias=use_bias
    )
    conv4_1 = u_net_down_block(
        conv3_1, index=4, padding_mode=padding_mode, use_bias=use_bias
    )

    time_flat = flatten_time(conv4_1, padding_mode=padding_mode, use_bias=use_bias)

    # upsampling path
    conv5_1 = u_net_up_block(
        time_flat, concat_tensor=conv3_1, padding_mode=padding_mode, use_bias=use_bias
    )
    conv6_1 = u_net_up_block(
        conv5_1, concat_tensor=conv2_1, padding_mode=padding_mode, use_bias=use_bias
    )
    conv7_1 = u_net_up_block(
        conv6_1, concat_tensor=conv1_1, padding_mode=padding_mode, use_bias=use_bias
    )
    conv8_1 = u_net_up_block(
        conv7_1, concat_tensor=conv0_1, padding_mode=padding_mode, use_bias=use_bias
    )
    outputs = ks.layers.Conv2D(
        filters=1,
        kernel_size=1,
        padding=padding_mode,
        activation=None,
